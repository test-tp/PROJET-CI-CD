...
Contrôle des vulnérabilités (Scan de sécurité) : Intégration d'un outil d'analyse dans la CI pour scanner l'image Docker. Si une faille critique (CVE) est détectée, le pipeline se bloque pour empêcher le déploiement d'une image vulnérable.
